        ...
        if res == None:
            if len(username) > 3:
                if len(username) < 12:
                    ...
                    cur.execute("INSERT INTO users (id, username, password, friends, banned, rank, reason, character, sbtoken, twostep, email, verified, code, ip, claimedemail, sbcoins) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (newid, username, password, "[]", "False", 1, None, None, newtoken, "False", None, "False", 0000, request.environ.get('HTTP_X_REAL_IP', request.remote_addr), "False", 0))
                    con.commit()
                    response = make_response(redirect(url_for("home")))
            ...
            if reason == "" or None:
                ...
                if ip_rank == None:
                    ...
                    if ip_rank[1] >= val[1]:
                        ...
                        cur.execute(f"SELECT ip FROM ipbans WHERE ip='{ip_rank[0]}'")
                        res = cur.fetchone()
                        if not res == None:
...
def redeem():
    if request.method == "POST":
        ...
        if val == False:
            ...
            cur.execute(f"SELECT uses, amount FROM codes WHERE code='{code}'")
            res = cur.fetchone()
            if res == None:
                return render_template("redeemer.html", color="red", message="Code is invalid or already used!")
            else:
                uses = res[0]
                amount = res[1]
                cur.execute(f"SELECT sbcoins FROM users WHERE username='{val}'")
                curamount = cur.fetchone()[0]
                cur.execute(f"UPDATE users SET sbcoins={curamount + amount}")
                print(uses)
                if uses == 1 :
                    cur.execute(f"DELETE FROM codes WHERE code='{code}'")
                con.commit()
                return render_template("redeemer.html", color="green", message=f"{amount} sbcoins have been added to your account!")
...
def ranker():
    if request.method == 'POST':
        ...
        if val == False:
            ...
            usertochange = request.form.get("username")
            newrank = request.form.get("rank")

            if newrank == "" or None:
                return render_template("ranker.html", username=val[0], color="red", message="Username/rank cannot be left blank.")
            else:
                cur.execute(f"SELECT rank FROM users WHERE username='{usertochange}'")
                rank = cur.fetchone()
                if rank == None:
                    return render_template("ranker.html", username=val[0], color="red", message=f"User {usertochange} does not exsist.")
                else:
                    if rank[0] >= val[1]:
                        return render_template("ranker.html", username=val[0], color="red", message="Cannot change rank user with more or equal power than you.")
                    else:
                        cur.execute(f"UPDATE users SET rank='{int(newrank)}' WHERE username='{usertochange}'")
                        return render_template("ranker.html", username=val[0], color="green", message=f"Changed {usertochange}'s rank to {newrank}.")
    else:
        val = validate_admin()
        if val == False:
            return abort(404)
        else:
            return render_template("ranker.html", username=val[0])     
...
def create_code():
    if request.method == 'POST':
        ...
        if val == False:
            ...
            if amount == "" or None:
                return render_template("createcode.html", username=val[0], color="red", message="Amount cannot be left blank.")
            else:
                if uses == None or "" or 0 or "0":
                    uses = 1
                else:
                    uses = int(uses)
            newcode = code()
            cur.execute("INSERT INTO codes (amount, uses, code) VALUES (?, ?, ?)",(int(amount), uses, newcode))
            return render_template("createcode.html", username=val[0], color="green", message=f"A code: {newcode}, was created with {amount} sbcoins.")
        ...
        if val == False:
            ...
            return render_template("createcode.html", username=val[0])